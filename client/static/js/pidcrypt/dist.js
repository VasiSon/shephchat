...
if (typeof pidCrypt != 'undefined' && typeof pidCrypt.AES != 'undefined' && typeof pidCrypt.MD5 != 'undefined') {
  ...
  */
  pidCrypt.AES.CBC.prototype.decrypt = function (ciphertext) {
    var pidcrypt = this.pidcrypt;
    var p = pidcrypt.getParams(); //get parameters for operation set by init
    if (ciphertext) pidcrypt.setParams({ dataIn: ciphertext });
    if (!p.decryptIn) {
    ...
    if (p.UTF8) plaintext = pidCryptUtil.decodeUTF8(plaintext); // decode from UTF8 back to Unicode multi-byte chars
